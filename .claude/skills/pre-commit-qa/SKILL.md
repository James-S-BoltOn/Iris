...
## 3. Internationalization (i18n) Adherence

User-facing text must not be hard-coded in frontend components. All labels,
messages, tooltips, and user-visible strings belong in the localization files.

**Localization file paths:**
- English: `frontend/src/locales/en/`
- Spanish: `frontend/src/locales/es/`

**Rules:**
- If you added or changed any user-facing string, the corresponding key must
  exist in both `en` and `es` locale files.
- If you added a key to `en`, you must also add it to `es` (even if the
  translation is a placeholder or TODO — the key must exist so the app doesn't
  break in Spanish).
- Grep the frontend source for raw string literals in JSX/TSX render output. If
  you find any that aren't i18n keys, flag them.

**Evidence required:** List the locale keys you added or modified. Confirm both
`en` and `es` files were updated. If no user-facing strings were changed, state
that explicitly.
...
`docs/ignored/` after completion. This keeps the active spec directory clean for
...
| Data Dictionary | `docs/DATA_DICTIONARY.md` | Database schema, models, or data structures changed |
| API Reference | `docs/API.md` | Endpoints, request/response shapes, or auth changed |
| System Diagram | `docs/system-diagram.md` | Architecture, services, or data flow changed |
...
- For agent utility scripts, "reviewed" means executed — not eyeballed. Run each
  ...
  script.
- For the others, if no update is needed, state why (e.g., "No schema changes,
...
| 3 | i18n Adherence       | ✅ / ❌ | [brief note]                  |
| 4 | Spec Archived        | ✅ / ⬜ | [⬜ = N/A, no spec]           |
| 5 | Docs Updated         | ✅ / ❌ | [which docs touched]          |
| 6 | Commit Plan Approved | ✅ / ❌ | [number of planned commits]   |
| 7 | Git Tracking         | ✅ / ❌ | [unexpected files? clean tree?]|
```
